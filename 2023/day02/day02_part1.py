...
def valid_games(data : str, bag_contents : dict):
    ...
    for line in data.split(LINE_END):
        ...
        #print(f"Line '{line}' : {game_number}, {colors}")
        # Check if this game's colors are <= the bag
        if all([colors[c] <= BAG_CONTENTS[c] for c in Colors]):
            # This game is valid
...
def main():
    ...
    result = sum(valid_games(data, BAG_CONTENTS))
    print(result)
