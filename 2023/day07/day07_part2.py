...
def kind(descriptor) -> HandType:
    ...
    groups = Counter(descriptor)
    # Do not remove 'J' if it is the only one
    ...
    # Add J to the biggest group
    sorted_groups[-1] = (sorted_groups[-1][0], sorted_groups[-1][1]+Js)
    sizes = tuple([x[1] for x in sorted_groups])
