...
def simulate_light_path(matrix):
    energized_matrix = np.zeros_like(matrix)
    positions_directions = [(Position(0,0), Direction(1,0))]
    while True:
...
def print_matrix(matrix, energize_matrix):
    for ml, eml in zip(matrix, energize_matrix):
        for mx, emx in zip(ml, eml):
            if emx > 0:
                print(Style.RESET_ALL + Fore.WHITE + Style.BRIGHT, end='')
            else:
                print(Fore.LIGHTBLACK_EX + Style.DIM, end='')
            print(ELEMENT_MAP_INV[mx], end='')
        print()
...
def main():
    ...
    with open(file) as f:
        ...
        energize_matrix = simulate_light_path(matrix)
        print_matrix(matrix, energize_matrix)
        print(np.sum(energize_matrix != 0))
