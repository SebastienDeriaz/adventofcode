        ...
        if network.nodes[node]['start']:
            ...
            for a, b in permutations(neighbors, 2):
                ...
                if offset[0] != 0 and offset[1] != 0:
                    ...
                    cost = 2*FORWARD_COST
                network.add_edge(a, b, weight=cost, shortcut=node)
...
def main():
    ...
    #total, path = nx.single_source_dijkstra(network, node_name(start), node_name(end))

    

    cells = set()
    shortcut_cells = set()
    #for path in all_paths(network, node_name(start), node_name(end), total):
    for path in nx.all_shortest_paths(network, node_name(start), node_name(end), weight='weight', method='dijkstra'):
        for a, b in zip(path[:-1], path[1:]):
            shortcut = network[a][b].get('shortcut', None)
            if shortcut is not None:
                #print(f'Add shortcut : {shortcut}')
                shortcut_cells.add(shortcut)
        path = set(path)
        cells = cells | path

    print(len(cells) + len(shortcut_cells))

    return
    plt.figure(figsize=(40,40))
    pos = {node : node_pos(node) for node in network.nodes}
    nodes = set(network.nodes)
    nx.draw_networkx_nodes(network, pos, nodelist = nodes - cells, node_size=20)
    nx.draw_networkx_nodes(network, pos, nodelist = cells, node_color='red', node_size=20)
